...
QStringList Browser::exec(QString sqlquery)
{
    QSqlQueryModel *model = new QSqlQueryModel(table);
    ...
    for(int i=0;;i++)
    {
        item=model->data(table->model()->index(i,0)).toString();
        if(item.isEmpty())
            break;
        else
...
void Browser::addConnection()
{
    ...
    Database dialog;
    dialog.driverName="QSQLITE";
    dialog.databaseName="barry963";
    dialog.hostName="barry963";
    dialog.userName="barry963";
    dialog.password="barry963";
    dialog.port=0;
